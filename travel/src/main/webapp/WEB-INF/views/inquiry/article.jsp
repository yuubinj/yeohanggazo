					...
					<div class="row p-2">
								...
								<c:choose>
									<c:when test="${prevDto.secret==1}">
										<c:if test="${sessionScope.member.userId==prevDto.userId || sessionScope.member.userLevel >= 51}">
											<a href="${pageContext.request.contextPath}/inquiry/article?num=${prevDto.num}&${query}">${prevDto.title}</a>
										</c:if>
										<c:if test="${sessionScope.member.userId!=prevDto.userId && sessionScope.member.userLevel < 51}">
											비밀글 입니다.
										</c:if>
								...
								<c:choose>
									<c:when test="${nextDto.secret==1}">
										<c:if test="${sessionScope.member.userId==nextDto.userId || sessionScope.member.userLevel >= 51}">
											<a href="${pageContext.request.contextPath}/inquiry/article?num=${nextDto.num}&${query}">${nextDto.title}</a>
										</c:if>
										<c:if test="${sessionScope.member.userId!=nextDto.userId && sessionScope.member.userLevel < 51}">
											비밀글 입니다.
										</c:if>
						...
						<div class="col-md-6 p-2 ps-0">
							<c:if test="${sessionScope.member.userId==dto.userId && empty dto.answer}">
								<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/inquiry/update?num=${dto.num}&page=${page}';">수정</button>
							</c:if>
