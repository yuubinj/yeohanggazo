		...
		<div class="body-container row justify-content-center">
			<div class="col-md-10 my-3 p-3">
				<div class="body-title">
					<h3>
						<!-- 아이콘 + 텍스트 출력 -->
						<i class="bi bi-megaphone-fill text-danger me-1"></i> 공지 게시글
						
						<!-- 값은 고정된 hidden input으로 넘김 -->
						...
						<tbody>
							...
							<tr>
								<td colspan="2">
								  ...
								  <c:url var="nextLink" value="/admin/bbs/article">
								...
								<tr>
								  ...
								  <c:url var="prevLink" value="/admin/bbs/article">
							...
							<td width="50%">
								<c:if test="${sessionScope.member != null}">
									<c:if test="${sessionScope.member.userId == dto.userId}">
										<!-- 본인 글일 경우 수정/삭제 가능 -->
										<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/admin/bbs/update?num=${dto.num}&categoryNum=1&page=${page}';">수정</button>
										<button type="button" class="btn btn-light" onclick="deleteBoard();">삭제</button>
									</c:if>
									<c:if test="${sessionScope.member.userId != dto.userId && sessionScope.member.userLevel >= 51}">
										<!-- 관리자만 삭제 가능 -->
										<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/admin/bbs/update?num=${dto.num}&categoryNum=1&page=${page}';">삭제</button>
									</c:if>
								</c:if>
							</td>
							<td class="text-end">
								<button type="button" class="btn btn-light" onclick="location.href='${pageContext.request.contextPath}/admin/bbs/list?${query}';">리스트</button>
							</td>
						</tr>
					</table>
	...
	<script type="text/javascript">
		function deleteBoard() {
		    if(confirm('게시글을 삭제 하시겠습니까 ?')) {
			    ...
			    let url = '${pageContext.request.contextPath}/admin/bbs/delete?' + params;
		    	location.href = url;
		    }
		}
	</script>
